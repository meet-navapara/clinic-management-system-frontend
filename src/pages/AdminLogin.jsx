...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (errors.email || errors.password) {
      ...
      document.getElementById(`admin-login-${firstKey}`)?.focus();
          ...
          <fieldset disabled={loading} className="space-y-3 sm:space-y-4 border-0 p-0 m-0 min-w-0">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  aria-describedby={fieldErrors.email ? 'admin-login-email-error' : undefined}
                />
              </div>
              {fieldErrors.email && (
                <p id="admin-login-email-error" className="text-xs text-red-600 mt-1" role="alert">
                  {fieldErrors.email}
                </p>
              )}
            </div>
            <div>
              ...
              {fieldErrors.password && (
                <p id="admin-login-password-error" className="text-xs text-red-600 mt-1" role="alert">
                  {fieldErrors.password}
                </p>
              )}
            </div>
